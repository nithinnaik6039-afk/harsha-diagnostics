...
if (isFirebaseConfigured) {
  try {
    ...
    auth = getAuth(app);
    console.log('[Firebase MLT] Initialized successfully.');
  } catch (error) {
    console.error('[Firebase MLT] Initialization failed:', error);
  }
} else {
  console.log('[Firebase MLT] Config has placeholders. Using local auth fallback.');
}
